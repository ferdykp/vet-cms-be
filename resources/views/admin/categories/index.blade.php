                ...
                <div x-data="{ editing: false }" class="border-b border-[#EEF1ED] last:border-b-0">
                    <div
                        ...
                        <div class="min-w-0">
                            <div class="flex items-center gap-2"><span class="size-2.5 rounded-full"
                                    style="background:{{ $category->accent_color ?: '#8FA58F' }}"></span><strong
                                    class="truncate text-sm text-[#293C32]">{{ $category->name }}</strong></div>
                            <p class="mt-1 truncate text-[11px] text-[#8A908A]">/{{ $category->slug }} ·
                                {{ \Illuminate\Support\Str::limit($category->description, 70) }}</p>
                        </div>
                        <span class="text-xs text-[#526A5A]">{{ $category->posts_count }}</span>
                        <span
                            class="w-fit rounded-full px-2.5 py-1 text-[9px] font-semibold {{ $category->is_active ? 'bg-[#E4EFE5] text-[#355A40]' : 'bg-[#F0F1EF] text-[#777]' }}">{{ $category->is_active ? 'ACTIVE' : 'HIDDEN' }}</span>
                    ...
                    <form x-show="editing" x-transition x-cloak method="POST"
                        ...
                        <label class="text-[10px] font-medium uppercase text-[#70766F] md:col-span-2">Description
                            <textarea class="mt-1 min-h-20 w-full rounded-lg border border-[#DDE2DF] bg-white px-3 py-2 text-xs" name="description">{{ $category->description }}</textarea>
                        </label>
