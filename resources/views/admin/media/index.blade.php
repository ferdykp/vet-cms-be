...
<section class="mb-7 flex flex-col items-start justify-between gap-5 lg:flex-row lg:items-end">
  ...
  <form method="POST" action="{{ route('admin.media.store') }}" enctype="multipart/form-data">@csrf<label class="inline-flex h-10 cursor-pointer items-center rounded-lg bg-[#526A5A] px-4 text-sm font-medium text-white shadow-sm hover:bg-[#293C32]"><x-icon name="plus" /> Upload Media<input type="file" name="files[]" multiple accept="image/jpeg,image/png,image/webp,image/gif,.pdf,.doc,.docx" class="sr-only" onchange="if(this.files.length) this.form.requestSubmit()"></label></form>
</section>
...
<div class="mb-5 flex flex-col gap-3 rounded-2xl border border-[#E6EAE4] bg-white p-3 shadow-sm sm:flex-row sm:items-center sm:justify-between">
  <form class="flex flex-1 flex-wrap gap-2"><div class="flex min-w-0 flex-1 items-center rounded-lg border border-[#DDE2DF] bg-white px-3"><span class="text-[#5B675E]"><x-icon name="search" /></span><input class="w-full border-0 bg-transparent px-2 py-2.5 text-sm outline-none" name="search" value="{{ request('search') }}" placeholder="Search filename or alt text"></div><select class="rounded-lg border border-[#DDE2DF] bg-white px-3 py-2 text-sm" name="filter"><option value="">All media</option><option value="images" @selected(request('filter')==='images')>Images</option><option value="documents" @selected(request('filter')==='documents')>Documents</option></select><button class="rounded-lg border border-[#DDE2DF] bg-[#F7F9F6] px-4 py-2 text-sm font-medium text-[#293C32]">Apply</button></form>
...
<div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]" x-data="mediaInspector()">
  <div>
    <div class="grid grid-cols-1 min-[360px]:grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5">
      ...
      <button type="button" class="group overflow-hidden rounded-xl border border-[#E6EAE4] bg-white text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#8FA58F] hover:shadow-md" @click='select({{ Js::from(['id'=>$asset->id,'name'=>$asset->original_name,'url'=>$asset->url,'size'=>$asset->size,'width'=>$asset->width,'height'=>$asset->height,'alt'=>$asset->alt_text,'caption'=>$asset->caption,'mime'=>$asset->mime_type,'extension'=>$asset->extension]) }})'>
        <div class="aspect-square overflow-hidden bg-[#EEF3ED]">@if(str_starts_with($asset->mime_type ?? '','image/'))<img src="{{ $asset->url }}" alt="{{ $asset->alt_text ?: $asset->original_name }}" class="h-full w-full object-cover transition duration-300 group-hover:scale-[1.02]">@else<div class="grid h-full place-items-center"><span class="rounded-lg bg-white px-3 py-2 text-sm font-semibold text-[#526A5A]">{{ strtoupper($asset->extension ?? 'FILE') }}</span></div>@endif</div>
        <div class="p-3"><strong class="block truncate text-[14px] text-[#293C32]">{{ $asset->original_name }}</strong><span class="mt-1 block truncate text-[12px] text-[#5B675E]">{{ $asset->alt_text ?: ($asset->mime_type ?? 'asset') }}</span></div>
      </button>
      @empty<div class="col-span-full rounded-2xl border border-dashed border-[#C9D3C9] bg-[#F7FAF7] p-12 text-center"><p class="text-sm font-medium text-[#526A5A]">No media yet</p><p class="mt-1 text-sm text-[#5B675E]">Upload your first image or document using the button above.</p></div>@endforelse
    </div>
  ...
  <aside x-show="selected" x-transition.opacity x-cloak @keydown.escape.window="selected=null" class="fixed inset-3 z-[70] overflow-y-auto self-start max-h-[calc(100dvh-24px)] rounded-2xl xl:static xl:z-auto xl:max-h-[calc(100dvh-120px)] border border-[#E6EAE4] bg-white p-5 shadow-sm xl:sticky xl:top-24">
    <template x-if="selected"><div>
      ...
      <div class="mt-4 flex items-center justify-between border-t border-[#EEF1ED] pt-4"><a :href="selected.url" target="_blank" rel="noopener noreferrer" class="text-[14px] font-medium text-[#526A5A] no-underline">Open original ↗</a><form method="POST" :action="'/admin/media/' + selected.id" onsubmit="return confirm('Delete this media asset? This will be blocked if it is still in use.')">@csrf @method('DELETE')<button class="text-[14px] font-medium text-[#B24E42]">Delete</button></form></div>
    </div></template>
  </aside>
  <aside x-show="!selected" class="hidden self-start rounded-2xl border border-dashed border-[#C9D3C9] bg-[#F7FAF7] p-7 text-center xl:block xl:sticky xl:top-24"><div class="mx-auto grid size-11 place-items-center rounded-xl bg-white text-xl text-[#526A5A]"><x-icon name="image" /></div><h2 class="mt-3 text-sm font-semibold text-[#293C32]">Select an asset</h2><p class="mt-1 text-sm leading-5 text-[#5B675E]">Click any media card to inspect dimensions, edit alt text and caption, or remove unused files.</p></aside>
</div>
@endsection
