...
<?php $__env->startSection('content'); ?>
    <section class="flex flex-col items-start justify-between gap-6 mb-7 lg:flex-row">
        <div><span class="text-[10px] font-semibold tracking-[0.09em] text-[#526A5A]">JOURNAL & KNOWLEDGE ARCHIVE</span>
            <h1>Journal</h1>
            <p>Write, organize, and publish clinical knowledge, reflections, and stories.</p>
        </div><a
            class="inline-flex h-[38px] cursor-pointer items-center justify-center rounded-md border border-transparent px-3.5 text-xs font-medium no-underline transition bg-[#526A5A] text-white hover:bg-[#293C32]"
            href="<?php echo e(route('admin.posts.create')); ?>">＋ New Content</a>
    </section>
    <div class="mb-4 rounded-lg border border-[#EBEFEA] bg-white p-2.5">
        <form
            class="flex flex-wrap items-center gap-2 [&_input]:rounded-md [&_input]:border [&_input]:border-[#DDE2DF] [&_input]:bg-white [&_input]:px-2.5 [&_input]:py-2 [&_input]:text-xs [&_input]:outline-none [&_select]:rounded-md [&_select]:border [&_select]:border-[#DDE2DF] [&_select]:bg-white [&_select]:px-2.5 [&_select]:py-2 [&_select]:text-xs [&_select]:outline-none [&_input:focus]:border-[#526A5A] [&_select:focus]:border-[#526A5A]">
            ...
            <button
                class="inline-flex h-[38px] cursor-pointer items-center justify-center rounded-md border border-transparent px-3.5 text-xs font-medium no-underline transition border-[#DDE2DF] bg-white text-[#293C32] hover:bg-[#F7F5EF]">Filter</button>
        </form>
    </div>
    <div class="overflow-hidden rounded-lg border border-[#EBEFEA] bg-white">
        <div
            class="hidden grid-cols-[minmax(260px,1.6fr)_.8fr_.6fr_.7fr_1fr] items-center gap-[15px] bg-[#F0F5F0] px-4 py-3 text-[9px] uppercase tracking-[0.06em] text-[#70766F] md:grid">
            <span>Title</span><span>Category</span><span>Status</span><span>Updated</span><span></span>
        </div>
        <?php $__empty_1 = true; $__currentLoopData = $posts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $post): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div
                class="grid grid-cols-1 items-center gap-2 border-t border-[#EBEFEA] px-4 py-3 text-xs md:grid-cols-[minmax(260px,1.6fr)_.8fr_.6fr_.7fr_1fr] md:gap-[15px] [&_small]:mt-1 [&_small]:block [&_small]:text-[#70766F]">
                <div
                    class="flex items-center gap-2.5 [&_img]:h-[38px] [&_img]:w-11 [&_img]:rounded [&_img]:object-cover [&_a]:block [&_a]:font-semibold [&_a]:text-[#181D1A] [&_a]:no-underline [&_small]:mt-1 [&_small]:block [&_small]:text-[#70766F]">
                    <?php if($post->featuredMedia): ?>
                    <img src="<?php echo e($post->featuredMedia->url); ?>" alt=""><?php else: ?><div
                            class="grid h-[38px] w-11 place-items-center rounded bg-[#E7ECE5]">▤</div>
                    <?php endif; ?>
                    <div>
                        <a
                        ...
                        class="rounded-full px-2 py-1 text-[9px] font-semibold <?php echo e(match ($post->status) {'published' => 'bg-[#526A5A] text-white','scheduled' => 'bg-[#FFF0DC] text-[#8B5D21]','archived' => 'bg-[#EFEFEF] text-[#777]',default => 'bg-[#E7ECE5] text-[#3A5243]'}); ?>"><?php echo e(strtoupper($post->status)); ?></span></span><span><?php echo e($post->updated_at->diffForHumans()); ?></span>
                <div
                    class="flex items-center gap-2 md:justify-end [&_a]:text-[10px] [&_a]:font-medium [&_a]:text-[#526A5A] [&_a]:no-underline [&_button]:border-0 [&_button]:bg-transparent [&_button]:text-[10px] [&_button]:font-medium [&_button]:text-[#526A5A]">
        ...
        </div><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?><div class="p-10 text-center text-[#70766F]">
