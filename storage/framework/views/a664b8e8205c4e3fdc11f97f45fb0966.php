...
<section class="rounded-2xl border border-[#E6EAE4] bg-white p-5 shadow-sm sm:p-6">
  <div class="mb-5 border-b border-[#EEF1ED] pb-3"><h2 class="text-sm font-semibold capitalize text-[#293C32]"><?php echo e(str_replace('_',' ',$group)); ?></h2><p class="mt-1 text-sm text-[#5B675E]"><?php echo e(match($group) { 'general' => 'Basic site identity and contact details.', 'publishing' => 'Defaults for journal presentation.', 'seo' => 'Search engine defaults for the public site.', 'contact' => 'Public contact form behavior.', default => 'Configuration values for this section.' }); ?></p></div>
  ...
  <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $setting): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    <div class="<?php echo e($setting->type==='text' ? 'md:col-span-2' : ''); ?>">
      ...
      <?php if($setting->type==='boolean'): ?>
        <label class="flex items-start justify-between gap-5 rounded-xl border border-[#EEF1ED] bg-[#FAFBF9] p-4"><span><strong class="block text-sm font-medium text-[#293C32]"><?php echo e($pretty); ?></strong><small class="mt-1 block text-[13px] leading-4 text-[#5B675E]"><?php echo e($setting->key); ?></small></span><span class="relative"><input type="hidden" name="settings[<?php echo e($i); ?>][value]" value="0"><input type="checkbox" name="settings[<?php echo e($i); ?>][value]" value="1" class="size-4 accent-[#526A5A]" <?php if($setting->typed_value): echo 'checked'; endif; ?>></span></label>
      <?php elseif($setting->type==='text'): ?>
        <label class="block text-sm font-medium text-[#424843]"><?php echo e($pretty); ?><textarea name="settings[<?php echo e($i); ?>][value]" class="mt-1 min-h-24 w-full rounded-lg border border-[#DDE2DF] bg-white px-3 py-2.5 text-sm outline-none focus:border-[#8FA58F]"><?php echo e($setting->value); ?></textarea><small class="mt-1 block text-[13px] text-[#5B675E]"><?php echo e($setting->key); ?></small></label>
      <?php else: ?>
        <label class="block text-sm font-medium text-[#424843]"><?php echo e($pretty); ?><input name="settings[<?php echo e($i); ?>][value]" type="<?php echo e(in_array($setting->type,['integer','float']) ? 'number' : 'text'); ?>" value="<?php echo e($setting->value); ?>" class="mt-1 w-full rounded-lg border border-[#DDE2DF] bg-white px-3 py-2.5 text-sm outline-none focus:border-[#8FA58F]"><small class="mt-1 block text-[13px] text-[#5B675E]"><?php echo e($setting->key); ?></small></label>
      <?php endif; ?>
    </div>
    <?php ($i++); ?>
...
<div class="rounded-2xl border border-dashed border-[#C9D3C9] bg-[#F7FAF7] p-10 text-center"><h2 class="font-['Newsreader'] text-2xl text-[#293C32]">No website settings yet</h2><p class="mt-2 text-sm text-[#545F57]">Run <code class="rounded bg-white px-1.5 py-1">php artisan db:seed --class=WebsiteSettingSeeder</code> once.</p></div>
<?php endif; ?>
