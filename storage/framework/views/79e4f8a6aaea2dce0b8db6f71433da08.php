            ...
            <?php $__empty_1 = true; $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div x-data="{ editing: false }" class="border-b border-[#EEF1ED] last:border-b-0">
                    <div
                        ...
                        <div class="min-w-0">
                            <div class="flex items-center gap-2"><svg class="size-2.5 shrink-0" viewBox="0 0 10 10" aria-hidden="true"><circle cx="5" cy="5" r="5" fill="<?php echo e($category->accent_color ?: '#8FA58F'); ?>" /></svg><strong
                                    class="truncate text-sm text-[#293C32]"><?php echo e($category->name); ?></strong></div>
                            <p class="mt-1 truncate text-[14px] text-[#5B675E]">/<?php echo e($category->slug); ?> ·
                                <?php echo e(\Illuminate\Support\Str::limit($category->description, 70)); ?></p>
                        </div>
                        <span class="text-sm text-[#526A5A]"><?php echo e($category->posts_count); ?></span>
                        <span
                            class="w-fit rounded-full px-2.5 py-1 text-[12px] font-semibold <?php echo e($category->is_active ? 'bg-[#E4EFE5] text-[#355A40]' : 'bg-[#F0F1EF] text-[#777]'); ?>"><?php echo e($category->is_active ? 'ACTIVE' : 'HIDDEN'); ?></span>
                    ...
                    <form x-show="editing" x-transition x-cloak method="POST"
                        ...
                        <label class="text-[13px] font-medium uppercase text-[#545F57] md:col-span-2">Description
                            <textarea class="mt-1 min-h-20 w-full rounded-lg border border-[#DDE2DF] bg-white px-3 py-2 text-sm" name="description"><?php echo e($category->description); ?></textarea>
                        </label>
